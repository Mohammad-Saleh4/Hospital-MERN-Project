...
import { Key, User,X,Menu, FastForward } from 'lucide-react'
...
<h1 className={navbarStyles.logoTitle}>Hospitex</h1>
...
<Link key={item.href} to={item.href} className={`${navbarStyles.navItem} ${isActive? navbarStyles.navItemActive : navbarStyles.navItemInactive}`}>{item.label}</Link>
...
<Link key={idx} to={item.href} onClick={()=>setIsOpen(false)} className={`${navbarStyles.mobileMenuItem} ${isActive? navbarStyles.mobileMenuItemActive : navbarStyles.mobileMenuItemInactive}`}>{item.label}</Link>
)
...
}} className={navbarStyles.mobileLoginButton}>Login</button>
